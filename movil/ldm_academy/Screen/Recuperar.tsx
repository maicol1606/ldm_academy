...
export default function Recuperar() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log(error);
      if (error.response) {
        Alert.alert('Error', `Hubo un problema con la solicitud: ${error.response.data.message || 'Error desconocido'}`);
      } else {
  ...
  return (
    <View style={{ padding: 20 }}>
      <Text style={{ fontSize: 24, fontWeight: 'bold', marginBottom: 20 }}>Recuperar Contraseña</Text>

      <TextInput
        style={{
          height: 40, borderColor: 'gray', borderWidth: 1, marginBottom: 20, paddingLeft: 10,
        }}
        placeholder="Código"
        value={user.codigo}
