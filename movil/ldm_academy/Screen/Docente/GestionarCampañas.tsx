...
const GestionarCampañas = () => {
  ...
  const handleSubmitEdit = async (e: any) => {
    e.preventDefault();
    try {
      const res = await axios.put(`http://192.168.56.1:3000/api/campanas/actualizarCampana/${CampañaEdit.id_campaña}`, CampañaEdit);
      if (res.status === 200) {
      ...
      <ScrollView>
        {campañas.map((campaña) => (
          <View key={campaña.id_campaña} style={{ flexDirection: 'row', marginBottom: 20 }}>
            <Image
              source={{ uri: `http://192.168.56.1:3000/img/campañas/${campaña.imagen}` }}
