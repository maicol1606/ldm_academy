...
const HomeEstudiante: React.FC<HomeEstudianteProps> = ({ navigation }) => {  
  ...
  const handlePostular = async () => {
    try {
      const response = await axios.post('http://192.168.1.14:3000/api/postulacion/agregarPostulacion', postulado);
          ...
          <View key={campana.id_campaña ?? index} style={styles.card}>
            ...
            <View style={styles.buttonGroup}>
              <TouchableOpacity
                ...
                onPress={() => {
                  setPostulado(prev => ({ id_usuario: prev.id_usuario, id_campaña: campana.id_campaña }));
