...
const EstudianteList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDocente = async () => {
      try {
        const res = await axios.get('http://192.168.1.14:3000/api/docentes/obtenerDocentes');
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              const res = await axios.delete(`http://192.168.1.14:3000/api/docentes/eliminarDocente/${id}`);
              if (res.status === 200) {
