...
const EstudianteList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchEstudiantes = async () => {
      try {
        const res = await axios.get('http://192.168.104.49:3000/api/estudiantes/llamarEstudiantes');
  ...
  const handleDelete = async (id: number) => {
    try {
      const res = await axios.delete(`http://192.168.104.49:3000/api/estudiantes/eliminarEstudiante/${id}`);
      if (res.status === 200) {
