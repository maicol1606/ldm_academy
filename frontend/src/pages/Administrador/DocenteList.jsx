...
const EstudianteList = () => {
  ...
  useEffect(() => {
    const fetchDocente = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_PUBLIC_API_DOMAIN}/api/docentes/obtenerDocentes`);
  ...
  const eliminarEstudiante = async (id) => {
    try {
      ...
      if (confirm.isConfirmed) {
        const res = await axios.delete(`${import.meta.env.VITE_PUBLIC_API_DOMAIN}/api/docentes/eliminarDocente/${id}`);
        if (res.status === 200) {
      ...
      <div className="container py-5">
        ...
        <div className="row g-4">
          ...
          {estudiantesFiltrados.length === 0 && (
            <div className="text-center text-">No se encontraron docentes.</div>
          )}
        </div>
