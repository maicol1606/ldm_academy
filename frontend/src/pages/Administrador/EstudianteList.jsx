...
const EstudianteList = () => {
  ...
  useEffect(() => {
    const fetchEstudiantes = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_PUBLIC_API_DOMAIN}/api/estudiantes/llamarEstudiantes`);
  ...
  const eliminarEstudiante = async (id) => {
    try {
      ...
      if (confirm.isConfirmed) {
        const res = await axios.delete(`${import.meta.env.VITE_PUBLIC_API_DOMAIN}/api/estudiantes/eliminarEstudiante/${id}`);
        if (res.status === 200) {
