...
const NotificacionesAdmin = () => {
    ...
    const handleAccept = (id) => {
        axios
            .put(`${import.meta.env.VITE_PUBLIC_API_DOMAIN}/api/notificaciones/${id}`, { estado: "Aceptado" })
            .then((res) => {
    ...
    const handleReject = (id) => {
        axios
            .put(`${import.meta.env.VITE_PUBLIC_API_DOMAIN}/api/notificaciones/${id}`, { estado: "Rechazado" })
            .then((res) => {
                setAlertType("danger");
    ...
    const togglePerfil = (id) => {
        setNotifications((prev) =>
            prev.map((n) => (n.id === id ? { ...n, perfilVisible: !n.perfilVisible } : n))
        );
    };
    ...
    const filteredNotifications = notifications.filter(
        (n) =>
            (searchTerm === "" ||
                (n.nombre_estudiante &&
                    n.nombre_estudiante.toLowerCase().includes(searchTerm.toLowerCase())) ||
                ...
                (n.idEstudiante &&
                    String(n.idEstudiante).toLowerCase().includes(searchTerm.toLowerCase()))) &&
        ...
        <section className="container-fluid bg-light min-vh-100 p-3">
            <div className="row">
                <div className="col-md-10 offset-md-2">
                    ...
                    <p className="text-muted">
                        Tienes {notifications.length} notificaciones por revisar
                    </p>
            ...
            {modalNotification && (
                <Modal show={showModal} onHide={closeModal}>
                    ...
                    <Modal.Body>
                        <p>
                            <strong>ID Estudiante:</strong> {modalNotification.idEstudiante}
                        </p>
                        <p>
                            <strong>ID Notificación:</strong> {modalNotification.id}
                        </p>
                        <p>
                            <strong>Nombre Estudiante:</strong>{" "}
                            {modalNotification.nombre_estudiante}
                        </p>
                        <p>
                            <strong>Campaña:</strong> {modalNotification.campaña}
                        </p>
                        <p>
                            <strong>Fecha de Postulación:</strong>{" "}
                            {modalNotification.fecha_postulacion}
                        </p>
                        <p>
                            <strong>Estado:</strong> {modalNotification.estado}
                        </p>
                    </Modal.Body>
                    <Modal.Footer>
