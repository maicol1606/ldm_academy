...
exports.mostrarPostulacionesPorDocente = (req, res) => {
    id = req.params.id
    const values = [id];
    db.query('SELECT campañas.*, postulacion.*, postulacion.estado as estado_postulacion, usuarios.* FROM postulacion INNER JOIN campañas ON postulacion.id_campaña = campañas.id_campaña INNER JOIN usuarios ON postulacion.id_usuario = usuarios.id_usuario WHERE id_docente = ?', values, (error, results) => {
        if (error) {
            console.error('Error al obtener las postulaciones:', error);
            res.status(500).json({ error: 'Error al obtener las postulaciones' });
        } else {
...
exports.aceptarPostulacion = (req, res) => {
    ...
    console.log(correo);
    const query = `UPDATE postulacion SET estado = 'aceptada' WHERE id_postulacion = ?`;
...
exports.rechazarPostulacion = (req, res) => {
    const id_postulacion = req.params.id_postulacion;
    const correo = req.body.correo;
    const query = `UPDATE postulacion SET estado = 'rechazada' WHERE id_postulacion = ?`;
    db.query(query, [id_postulacion], (error, results) => {
        if (error) {
